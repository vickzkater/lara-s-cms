...
@php
  ...
  if(isset($data)){
    ...
    $link_get_data = route('admin.usergroup.get_data');
    $function_get_data = 'refresh_data();';
  }else{
    $pagetitle = ucwords(lang('deleted #item', $translation, ['#item' => $this_object]));
    $link_get_data = route('admin.usergroup.get_data_deleted');
  ...
  <script>
    ...
    function refresh_data() {
      ...
      var table = $('#datatables').DataTable({
        ...
        ajax: "{{ $link_get_data }}",
        columns: [
          {data: 'name', name: 'sys_groups.name'},
          {data: 'division_name', name: 'division_name'},
          {data: 'item_status', name: 'item_status'},
          {data: 'created_at', name: 'sys_groups.created_at'},
          {data: 'updated_at', name: 'sys_groups.updated_at'},
    ...
    function refresh_data_deleted() {
      ...
      var table = $('#datatables-deleted').DataTable({
        ...
        columns: [
          ...
          {data: 'item_status', name: 'item_status'},
          {data: 'created_at', name: 'sys_groups.created_at'},
          {data: 'deleted_at', name: 'sys_groups.deleted_at'},
          {data: 'action', name: 'action'},
        ]
      });
    }
